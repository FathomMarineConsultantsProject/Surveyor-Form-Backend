...
export async function submitForm(req, res) {
    ...
    if (!photo)
        return res.status(400).json({ success: false, message: "photoFile is required" });
    if (!cv)
    ...
    const qualifications = parseJsonField(req.body.qualifications, []);
    const experienceByQualification = parseJsonField(req.body.experienceByQualification, {});
    const vesselTypes = parseJsonField(req.body.vesselTypes, []);
    const shoresideExperience = parseJsonField(req.body.shoresideExperience, []);
    const surveyingExperience = parseJsonField(req.body.surveyingExperience, []);
    const vesselTypeSurveyingExperience = parseJsonField(req.body.vesselTypeSurveyingExperience, []);
    const accreditations = parseJsonField(req.body.accreditations, []);
    const coursesCompleted = parseJsonField(req.body.coursesCompleted, []);
    const references = parseJsonField(req.body.references, []);
...
export async function approveForm(req, res) {
    const id = Number(req.params.id);
    if (!id)
        return res.status(400).json({ success: false, message: "Invalid id" });
